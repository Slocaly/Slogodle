...
description: Scan public/logos for SVG files that aren't yet registered in packages/logos/src/logos.ts, and add a clean, correctly-formatted entry for each one.
disable-model-invocation: true
allowed-tools: Read, Edit, Bash, Glob, Grep, WebSearch, WebFetch, AskUserQuestion
---

Add missing logos to `packages/logos/src/logos.ts`, keeping that file clean and every entry accurate. This skill only adds/repairs entries — it never touches game logic, components, or SVG files themselves.

## 1. Find what's new

- List every `*.svg` file in `public/logos/`.
- Extract every `icon: "/logos/....svg"` path already present in `packages/logos/src/logos.ts`.
...
Read the `Logo` interface at the top of `packages/logos/src/logos.ts` for the exact fields required. For each new file, determine:
...
- **description** — one factual, neutral sentence describing what the tool actually is/does — the kind of line you'd find at the top of its README or Wikipedia summary (e.g. "Redis is an open-source, in-memory key-value store used as a database, cache, and message broker."). This must NOT overlap with `funFact` — it states what the thing is, not a piece of trivia about it.
- **funFact** — one sentence, specific and verifiable trivia: a real origin detail, naming story, notable person involved, or surprising history — never a restatement of what the tool does or how it works (that belongs in `description`). Match the style of existing `funFact` values. Verify via WebSearch if you're not confident the fact is accurate; never fabricate a fact.
...
- Field order within each entry: `name`, `industry`, `founded`, `description`, `funFact`, `icon`, `aspect`, `gitLink`.
- For `description` and `funFact`, follow the file's existing convention: keep the value on one line (`description: "..."` / `funFact: "..."`) if it stays reasonably short like the majority of entries; when it runs long, wrap it like most entries do — the key alone on its line, the quoted string indented 6 spaces on the next line.
